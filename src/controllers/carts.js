...
const addProductToCart = async (req, res) => {
    ...
    if (!cid || !pid)
        CustomError.create({
            name: "Error al agregar producto a carrito",
            message: "Poner id de producto o carrito correcto",
            cause: "Id de producto o carrito incorrecto",
...
//Crear ticket de carrito
const purchase = async (req, res) => {
    const cid = req.params.cid;
    let cart = await cartsManager.getOne(cid)
    const purchaser = cart.user.email;
    const result = await cartsManager.purchase(cid,purchaser);
    res.send({ status: "Success", payload: result });
}
